...
export default function DonatePage() {
  ...
  useEffect(() => {
    const isValid = (
      formContent.name &&
      formContent.estimatedPrice > 0 &&
      formContent.category &&
      formContent.condition &&
      formContent.desc &&
    ...
    <article className='donate-container'>
      <main className='main-content'>
        <section className='form-donate'>
          ...
          {errorMsg ? <span className='error-msg'>{errorMsg}</span> : undefined}
          <button type='submit' form='donate-form' className={`submit-donate${loggedUser && isFormValid ? '' : ' hidden'}`}  onClick={ev => submitProduct(ev, formContent, initialFormContent, setErrorMsg, loggedUser as User)}>Cadastrar Doação</button>
        </section>
        <section className='donate-table'>
          <main className='table-content'>
            <h1 className='donate-title'>Sistema de Pontos</h1>
